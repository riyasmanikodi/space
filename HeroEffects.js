 ...
 * - SYSTEM: [PRO PHASE] Engineered multi-layered DOM structure for Space Realism glitches.
 * - SYSTEM: [PRO PHASE] Integrated BLOCK_SCRAMBLE, SLICE_DRIFT, and PHOSPHOR_SPLIT anomaly handlers.
 * - SYSTEM: [PRO PHASE] Resolved Identity string wrap failure via absolute layout injection.
 * - SYSTEM: [PRO PHASE] Expanded anomaly pool to 15 unique archetypes to complete the cycle requirements.
 * - SYSTEM: [PRO PHASE] Transitioned HeroEffects to a "Visual Slave" architecture. Centralized heartbeat to Logics.js.
 * - SYSTEM: [PRO PHASE] Recalibrated all internal animation loops to strictly terminate at 2.0 seconds.
 * - SYSTEM: [PRO PHASE] Restored RIYAS MANIKODI as the primary glitch target for MAIN mode.
 * - SYSTEM: [PRO PHASE] Preserved KRAYETOS isolation protocol for GREETING mode.
 ...
 * - SYSTEM: [PRO PHASE] Consolidated hardware gateway interaction to unified 'pointerdown' event.
 * * * * * CULPRIT LOG V28:
 ...
 * - FIXED [ID 4607]: [PRO PHASE] Layout Fragmentation. Injected `white-space: nowrap` and absolute inset positioning to force glitch ghost layers onto a single Z-axis plane.
 * - FIXED [ID 5635]: [PRO PHASE] WebGL Regression. Restored DOM-driven `clip-path` slicing and RGB screen blending for true cathode-ray aesthetics.
 * - FIXED [ID 6001]: [PRO PHASE] Transient Anomalies. Extended all JS-driven glitch loops and timeouts to match the new 2.0s duration requirement.
 * - FIXED [ID 6006]: [PRO PHASE] Repetition Error. Enforced 15-effect full-cycle depletion before reshuffling.
 * - FIXED [ID 6008]: [PRO PHASE] Double Queue Bug. Removed local shuffleQueue and heartbeat interval to prevent interaction desync with Logics.js.
 ...
 * - FIXED [ID 8005]: [PRO PHASE] Mobile Gateway Double-Tap. Replaced dual-binding ('click'/'touchstart') with a single 'pointerdown' listener to prevent double-incrementing the tap counter.
 ...
 * - Fixed: [PRO PHASE] Calibrated manifestation duration exactly to 2.0 seconds.
 * - Fixed: [PRO PHASE] Added NOISE_SHIVER to the glitch handler to support the 15th anomaly.
 * - Fixed: [PRO PHASE] Stripped startHeartbeat() to defer anomaly timing authority to the Master Brain (Logics.js).
 ...
 * - Fixed: [PRO PHASE] Removed redundant touchstart listener to resolve event collision on mobile displays.
 * * * * * RIPPLE EFFECT V28:
 ...
 * - RIPPLE: [PRO PHASE] The hero name now correctly maintains its inline footprint without wrapping to a second line during glitch states.
 * - RIPPLE: [PRO PHASE] Phosphor split realistically mimics electron gun convergence failure.
 ...
 * - RIPPLE: [PRO PHASE] The 8-tap developer gateway now correctly registers exactly one count per physical touch across all device profiles.
 * * * * * REALITY AUDIT V28:
 ...
 * - APPEND 4607: [PRO PHASE] Layout Synchronization - Verified pseudo-elements replaced with explicit DOM nodes to guarantee absolute positioning stability.
 * - APPEND 5635: [PRO PHASE] Performance - Verified hardware-accelerated `clip-path` slicing eliminates main-thread text reflows.
 * - APPEND 6001: [PRO PHASE] Duration Audit - Verified that `setTimeout` and `maxFrames` constants across all glitch handlers sum to exactly 2000ms.
 * - APPEND 6008: [PRO PHASE] Master Queue Audit - Verified HeroEffects acts solely on GLOBAL_GLITCH events, removing double-queue anomalies.
 ...
 * - APPEND 8005: [PRO PHASE] Gateway Interaction Audit - Verified 'pointerdown' correctly traps exactly 8 taps without overflow on iOS/Android.
...
export class HeroEffects {
     ...
     * @param {string} context - 'MAIN' or 'GREETING'
     */
    constructor(container, context = 'MAIN') {
    ...
    init() {
        ...
        this.container.innerHTML = `
            <div class="hero-identity-container" style="display: flex; flex-direction: row; align-items: center; justify-content: center; width: 100%;">
                <div class="hero-name-static" style="font-family: 'Courier New', monospace; font-weight: 400; letter-spacing: 6px; text-transform: uppercase; ${staticStyle}">
                    ${this.staticText}
                </div>
                <div class="hero-name-wrapper" data-text="${this.glitchText}">
                    <div class="layer-phosphor layer-phosphor-red" aria-hidden="true">${this.glitchText}</div>
                    <div class="layer-phosphor layer-phosphor-cyan" aria-hidden="true">${this.glitchText}</div>
                    <div class="layer-slice layer-slice-top" aria-hidden="true">${this.glitchText}</div>
                    <div class="layer-slice layer-slice-mid" aria-hidden="true">${this.glitchText}</div>
                    <div class="layer-slice layer-slice-btm" aria-hidden="true">${this.glitchText}</div>
                    <div class="layer-main">${this.glitchText}</div>
                </div>
            </div>
        `;
    ...
    bindEvents() {
        ...
        // [PRO PHASE] Unified Pointer Event for Hardware Gateway Interaction
        if (this.container) {
    ...
    applyIndustrialStyles() {
        ...
        if (cyan) { cyan.style.color = '#00f3ff'; cyan.style.mixBlendMode = 'screen'; cyan.style.transition = `transform ${ANOMALY_CONFIG.GHOST_DELAY || 0.1}s ease-out, opacity 0.2s`; }
    }
    ...
    runScramble(intensity, isBlock = false) {
        let frames = 0;
        const maxFrames = Math.floor((this.glitchDuration / 1000) * 60);
        const glyphs = ANOMALY_CONFIG ? ANOMALY_CONFIG.GLYPHS : ['█', '▓', '░', 'X'];
        const scrambleChars = isBlock ? glyphs : this.chars.split('');
        const density = (isBlock && ANOMALY_CONFIG) ? ANOMALY_CONFIG.ANOMALY_DENSITY : 0.4;

        const layers = this.container.querySelectorAll('.layer-main, .layer-slice, .layer-phosphor');

        const animate = () => {
            const mutatedText = this.glitchText.split('').map(char => {
                if (char === ' ') return ' ';
                return (Math.random() < density) ? scrambleChars[Math.floor(Math.random() * scrambleChars.length)] : char;
            }).join('');

            layers.forEach(layer => { layer.innerText = mutatedText; });

            frames++;
            if (frames < maxFrames) {
                requestAnimationFrame(animate);
            } else {
                layers.forEach(layer => { layer.innerText = this.glitchText; });
                const wrapper = this.container.querySelector('.hero-name-wrapper');
    ...
    executeSliceDrift(intensity) {
        const slices = this.container.querySelectorAll('.layer-slice');
        if (!slices.length) {
            this.isGlitched = false;
            return;
        }
        ...
        let frames = 0;
        const jitterSpeed = (ANOMALY_CONFIG && ANOMALY_CONFIG.SLICE_JITTER_SPEED) ? ANOMALY_CONFIG.SLICE_JITTER_SPEED : 50;
        const maxFrames = Math.floor(this.glitchDuration / jitterSpeed);
        ...
        const animate = () => {
            ...
            frames++;
            if (frames < maxFrames) {
                setTimeout(() => requestAnimationFrame(animate), jitterSpeed);
            } else {
                slices.forEach(slice => {
    ...
    triggerPhosphorSplit(intensity) {
        const red = this.container.querySelector('.layer-phosphor-red');
        const cyan = this.container.querySelector('.layer-phosphor-cyan');
        ...
        const opacity = (ANOMALY_CONFIG && ANOMALY_CONFIG.PHOSPHOR_OPACITY) ? ANOMALY_CONFIG.PHOSPHOR_OPACITY : 0.4;
        red.style.opacity = opacity.toString();
        cyan.style.opacity = opacity.toString();

        const offset = 20 * intensity;
        red.style.transform = `translateX(${offset}px)`;
        cyan.style.transform = `translateX(-${offset}px)`;

        setTimeout(() => {
            red.style.transform = `translateX(0px)`;
            cyan.style.transform = `translateX(0px)`;
            red.style.opacity = '0';
    ...
    triggerNoiseShiver(intensity) {
        ...
        const animate = () => {
            ...
            const xOffset = (Math.random() - 0.5) * 4 * intensity;
