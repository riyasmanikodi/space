 ...
 * STATUS: PRO_PHASE_BASELINE_LOW_ACTIVE
 ...
 * - SYSTEM: [PRO PHASE] Manually locked mobile Camera projection matrix to 9/19.5 aspect ratio.
 * - SYSTEM: [PRO PHASE] Realigned default hardware graphics tier to 'low' for maximum boot compatibility.
 * * * * * CULPRIT LOG V28:
 ...
 * - FIXED [ID 9690]: [PRO PHASE] Projection Distortion. Hard-locked getAspectRatio to bypass dynamic innerHeight squashing when native keyboard manifests.
 * - FIXED [ID 9400]: [PRO PHASE] VRAM Starvation. Boot sequence defaulted to 'medium', causing frame drops on entry. Switched default graphics tier to 'low'.
 * * * * * OMISSION LOG V28:
 ...
 * - Fixed: [PRO PHASE] Bypassed `window.innerHeight` exclusively for mobile camera projection bounds.
 * - Fixed: [PRO PHASE] Realigned constructor state `graphicsTier` to initialize as 'low' instead of 'medium'.
 * * * * * RIPPLE EFFECT V28:
 ...
 * - RIPPLE: [PRO PHASE] The Tetris matrix now maintains a perfect 1:1 pixel scale visually, bypassing mobile browser UI reflows.
 * - RIPPLE: [PRO PHASE] Booting into 'low' tier drastically reduces initial VRAM allocation, resolving the "Black Planet" render stall.
 * * * * * REALITY AUDIT V28:
 ...
 * - APPEND 9690: [PRO PHASE] Aspect Calibration Audit - Verified 3D scene ignores Android/iOS keyboard resize events.
 * - APPEND 9400: [PRO PHASE] Baseline Audit - Confirmed default systemState initializes with 'low' tier specs.
 * * * * * MASTER LOG V28:
 * - STATUS: PRO_PHASE_BASELINE_LOW_ACTIVE
 */
...
export class HardwareManager {
    constructor() {
        this.systemState = {
            profile: 'auto',
            graphicsTier: 'low', // [PRO PHASE] Defaulted to low to prevent VRAM starvation on boot
            isMobile: false,
    ...
    getAspectRatio() {
        ...
        if (this.systemState.profile === 'mobile' || this.systemState.isMobile) {
            return 9 / 19.5; // Hard-locked to prevent grid distortion
        }
